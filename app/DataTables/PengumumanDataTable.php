        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($data) {
                return sprintf(
                    ...
                    route('pengumuman.show', $data->slug),
                    $data->slug, // slug attribute for edit button
                    $data->slug, // slug attribute for delete button
                    $data->id,
                    route('pengumuman.edit', $data->slug),
                    // route('pengumuman.destroy', $data->slug),
                    method_field('DELETE'),
                    csrf_field()
                );
            })
        ...
        return [
            Column::computed('checkbox')
                ...
                ->render(function ($data) {
                    // Jika $data adalah objek pengumuman dan memiliki properti 'id'
                    if ($data instanceof \App\Models\Pengumuman && property_exists($data, 'id')) {
            ...
            Column::make('created_at')->title('Create At'),
