...
class GaleryVideoDataTable extends DataTable
{
    /**
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($data) {
                return sprintf(
                    ...
                    route('galery-video.show', $data->slug),
                    $data->slug, // slug attribute for edit button
                    $data->slug, // slug attribute for delete button
                    $data->id,
                    route('galery-video.edit', $data->slug),
                    // route('galery-video.destroy', $data->slug),
                    method_field('DELETE'),
                    csrf_field()
                );
            })
     ...
     */
    public function query(GaleryVideo $model)
    {
        ...
        return [
            Column::computed('checkbox')
                ...
                ->render(function ($data) {
                    // Jika $data adalah objek GaleryVideo dan memiliki properti 'id'
                    if ($data instanceof GaleryVideo && property_exists($data, 'id')) {
    ...
    {
        return 'GaleryVideo_' . date('YmdHis');
    }
}
